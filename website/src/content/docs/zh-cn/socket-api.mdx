---
title: Socket API
description: 从脚本、工具和编程 Agent 控制运行中的 Herdr 服务器。
---

Herdr 提供本地 socket API，供需要检查或控制运行中 Session 的脚本和 Agent 使用。

大多数自动化场景应从 CLI 封装命令入手。只有在需要直接的请求/响应控制，或长期事件订阅时，才使用原始 socket API。

## 选择集成层

| 层 | 用途 |
| --- | --- |
| Agent 技能 | 教编程 Agent 如何在 Pane 内使用 Herdr。 |
| CLI 封装 | Shell 脚本、简单编排和人工调试。 |
...
已安装的 CLI 可以打印该 Herdr 二进制内置的 socket 协议 schema：
...
直接运行 `herdr api schema` 打印简短摘要。`--json` 输出供工具使用的完整 JSON Schema 文档，`--output PATH` 将该文档写入文件。schema 覆盖原始请求、成功响应、错误响应、发出的事件和订阅事件。

## 你能控制什么

socket API 可以：

- 创建、列出、聚焦、重命名和关闭 Workspace
- 创建、列出、聚焦、重命名和关闭 Tab
- 列出、检查、分割、交换、聚焦、调整大小、重命名、读取、关闭 Pane 并向其发送输入
- 通过 CLI 辅助命令列出、检查、读取、发送、重命名、聚焦、启动和附加 Agent
- 从钩子和插件上报自定义 Agent 状态
- 订阅事件并等待输出或状态变化
- 安装和卸载内置集成
- 停止服务器并重载配置
...
一些 CLI 命令是这些方法的便捷封装。比如 `herdr agent wait` 先解析 Agent 目标，然后订阅 Pane Agent 状态事件。

`session.snapshot` 为自行维护本地运行时缓存的客户端返回一次性引导快照。响应包含版本/协议元数据、当前聚焦的 Workspace/Tab/Pane ID、Workspace 记录、Tab 记录、Pane 记录、Tab 布局快照和 Agent 记录。它不是订阅；读取后，应订阅资源事件，从事件更新本地缓存。重新连接后或本地缓存可能过期时，再次调用 `session.snapshot`。Workspace 记录上附带已关联的 worktree 来源信息。完整的仓库 worktree 发现仍用 `worktree.list`。

CLI 侧，`herdr api snapshot` 把实时的 `session.snapshot` 响应打印为 JSON，方便客户端和 Agent 做简单引导。

Pane 控制方法使用 `w1:p1` 这类公开 Pane ID。schema 中 `pane_id` 可选的方法，省略时使用服务器当前聚焦的活动 Pane。`pane.move` 始终要求提供来源 `pane_id`。
...
`pane.current` 返回单个 `PaneInfo`。带有 `caller_pane_id` 时，Herdr 返回那个 Pane。省略时，Herdr 返回当前聚焦的活动 Pane。
...
客户端可以把 `offset_from_bottom == 0` 视为已在底部的状态。

`pane.layout` 返回 Tab 布局快照，包含 `workspace_id`、`tab_id`、`zoomed`、外层 `area`、`focused_pane_id`、Pane 矩形和分割矩形/比例。`pane.neighbor` 和 `pane.edges` 也包含同一份布局快照，客户端无需私有布局状态就能做出下一步决策。

`pane.process_info` 返回 Pane 的 shell pid、可用时的前台进程组 ID，以及平台暴露时带有 pid、名称、argv/cmdline 和 cwd 的前台进程。

`layout.export` 返回可移植的 Tab 布局树。省略 `tab_id` 和 `pane_id` 导出活动 Tab，传 `tab_id` 导出该 Tab，或传 `pane_id` 导出包含该 Pane 的 Tab。

```json
{"id":"req_export","method":"layout.export","params":{"tab_id":"w1:t1"}}
```

响应包含 `workspace_id`、`tab_id`、`zoomed`、`focused_pane_id` 和 `root`。`root` 是由 `pane` 和 `split` 节点组成的 BSP 树。Pane 节点可以包含 `pane_id`、`label`、`cwd` 和 argv `command`。分割节点使用 `direction`（`right` 或 `down`）、`ratio`、`first` 和 `second`。

`layout.apply` 从声明式的树创建一个新 Tab。提供 `tab_id` 时，Herdr 先创建替代 Tab，再关闭旧 Tab。它会恢复结构、标签、cwd、env 和可选的 argv 命令；不会保留活跃的 PTY、回滚内容或运行中的进程。
...
`layout.set_split_ratio` 更新 Tab 布局中已有的分割。响应是 `type: "layout_split_ratio_set"`，附带更新后的可移植 `layout`。

```json
{"id":"req_ratio","method":"layout.set_split_ratio","params":{"tab_id":"w1:t1","path":[],"ratio":0.6}}
```

启动进程的方法接受一个 `env` 对象。Herdr 只把这些键值对应用到新启动的进程。Herdr 还向受管 Pane 进程注入 `HERDR_SOCKET_PATH`、`HERDR_ENV=1`、`HERDR_WORKSPACE_ID`、`HERDR_TAB_ID` 和 `HERDR_PANE_ID`。与调用方提供的环境变量冲突时，Herdr 管理的变量优先。
...
交换仅限同一 Tab。它保留分割形状、分割比例、Pane ID 和运行中的进程。响应是 `type: "pane_swap"`，带 `changed`、可选的 `reason`、`source_pane_id`、可选的 `target_pane_id`、`focused_pane_id` 和 `layout`。reason 的取值有 `no_neighbor`、`same_pane`、`not_found` 和 `cross_tab`。Tab 处于缩放状态时，交换保持缩放，并修改隐藏的整页布局。

`pane.move` 把运行中的 Pane 移动到另一个 Tab、新 Tab 或新 Workspace：
...
移动到已有 Tab 需要 `split: "right" | "down"`。`target_pane_id` 可选，默认是目标 Tab 的聚焦 Pane。同一 Tab 内的布局变化仍然用 `pane.swap`；移动到来源 Tab 返回 `changed: false` 和 `reason: "same_tab"`。涉及缩放状态的来源或目标 Tab 的移动返回 `changed: false` 和 `reason: "zoomed_tab"`。

响应是 `type: "pane_move"`，带 `changed`、可选的 `reason`、`previous_pane_id`、`previous_workspace_id`、`previous_tab_id`、被移动的 `pane`、可选的 `source_layout`、`target_layout`、可选的新建 Workspace 或 Tab 记录、可选的已关闭 Workspace 或 Tab ID，以及 `focused_pane_id`。跨 Workspace 移动保持内部 Pane 和终端存活，但在目标 Workspace 分配新的公开 Pane ID。订阅者可以监听 `pane.moved`；Herdr 不会为被移动的终端进程发出假的 Pane 关闭/创建事件。

`pane.zoom` 切换、启用或禁用目标 Pane 所在 Tab 的缩放：
...
省略 `pane_id` 时，目标是服务器当前聚焦的活动 Pane。响应是 `type: "pane_zoom"`，带 `changed`、`zoom_changed`、`focus_changed`、可选的 `reason`、`pane_id`、`focused_pane_id`、`zoomed` 和 `layout`。缩放状态或焦点任一发生变化时，`changed` 为 true。reason 的取值有 `single_pane`、`already_zoomed` 和 `already_unzoomed`。

`notification.show` 的 CLI 封装是：
...
Worktree 方法把 Git 检出作为 Herdr Workspace 管理。`worktree.create` 创建检出，并返回新的 `workspace`、`tab`、`root_pane` 和 `worktree` 记录。请求的分支已在本地存在时检出它；否则从请求的 base 或 `HEAD` 创建分支。`worktree.open` 打开已有检出，或返回已打开的 Workspace。`worktree.remove` 对关联的子 Workspace 运行 `git worktree remove`，从不删除分支。

从来源 Workspace 创建 worktree：
...
`worktree.list`、`worktree.create` 和 `worktree.open` 中，`workspace_id` 和 `cwd` 最多用一个；两者都省略则使用活动 Workspace。`worktree.open` 中，`path` 和 `branch` 恰好用一个。原始 socket 的 `cwd` 和 `path` 值必须是绝对路径；CLI 在发送请求前会展开相对的 `--cwd` 和 `--path` 值。当 Workspace 属于某个 Herdr worktree 组时，Workspace 响应包含可选的 `worktree` 来源信息。当已有 Workspace 获得或改变 worktree 来源信息时，worktree 命令可能发出 `workspace.updated`。

Worktree 命令也发出生命周期事件。`worktree.create` 发出 `workspace.created`、`tab.created`、`pane.created` 和 `worktree.created`。`worktree.open` 发出 `worktree.opened`，并在打开新的 Herdr Workspace 时同时发出 Workspace/Tab/Pane 创建事件。`worktree.remove` 发出 `worktree.removed`；如果关联的 Workspace 仍然打开，还会发出 `workspace.closed`。

## 插件 API

插件 API 是面向可执行工作流工具的早期宿主接口。插件是带 `herdr-plugin.toml` 清单的包。清单声明可分享的动作、事件钩子、终端 Pane 入口点和链接处理器。动作和 Pane 仅限清单声明；运行时动作注册和运行时 argv Pane 创建不在 v1 范围内。

安装和链接的插件跨重启持久化。在 `plugin.link`、`plugin.unlink`、`plugin.enable` 和 `plugin.disable` 时，Herdr 在 `session.json` 旁写入一个 `plugins.json` 注册表文件。Herdr 不在运行时，`herdr plugin install` CLI 也写同一个注册表，启动时自动加载。启动时，Herdr 从原始路径重新读取每个清单；文件缺失或无法解析时，条目会带着 `warnings` 字段保留，`plugin.list` 会将其展示出来。
...
在顶层用你的插件支持的操作系统标识（`linux`、`macos`、`windows`）声明 `platforms`。本地开发允许省略 `platforms` — `plugin.link` 会成功，但响应包含警告。单个构建命令、动作、事件钩子、Pane 和链接处理器可以声明自己的 `platforms` 来覆盖插件级列表；省略时从插件继承。调用有效 platforms 不包含当前操作系统的动作，或打开这样的 Pane，会返回 `platform_unsupported` 错误。
...
`plugin.action.invoke` 解析清单动作，启动清单命令，并返回 Herdr 构建的调用上下文和已启动命令的日志记录。缺失的上下文字段会从活动 Workspace、Tab、聚焦 Pane、worktree 来源信息和请求 ID 补全。调用被禁用插件的动作返回 `plugin_disabled` 错误。

Herdr 注入 `HERDR_SOCKET_PATH`、`HERDR_BIN_PATH`、`HERDR_ENV=1`、`HERDR_PLUGIN_ID`、`HERDR_PLUGIN_ROOT`、`HERDR_PLUGIN_CONFIG_DIR`、`HERDR_PLUGIN_STATE_DIR`、`HERDR_PLUGIN_CONTEXT_JSON`，以及可用的 `HERDR_WORKSPACE_ID`、`HERDR_TAB_ID` 和 `HERDR_PANE_ID` 值。动作命令还收到 `HERDR_PLUGIN_ACTION_ID`；事件钩子收到 `HERDR_PLUGIN_EVENT` 和 `HERDR_PLUGIN_EVENT_JSON`；Pane 命令收到 `HERDR_PLUGIN_ENTRYPOINT_ID`。
...
`plugin.pane.open` 要求已安装、已启用、平台兼容的插件，然后把请求的清单 `[[panes]]` 入口点作为 argv 支撑的终端 Pane 启动。清单 Pane 的 `placement` 默认为 `overlay`；请求的 `placement` 可以用 `overlay`、`split`、`tab` 或 `zoomed` 覆盖清单。覆盖层 Pane 针对活动 Pane。分割和缩放 Pane 针对已有 Pane；Tab Pane 可以针对 Workspace。Pane 打开后表现得像普通 Herdr Pane，但 `plugin.pane.focus` 和 `plugin.pane.close` 只作用于通过插件 API 打开的 Pane。focus 返回 `plugin_pane_focused`；close 返回 `plugin_pane_closed`。
...
对插件来说，需要可移植的 Windows 行为时，优先调用 `HERDR_BIN_PATH` 和 CLI 封装命令。原始 socket 客户端要自己负责使用平台原生的本地 socket 形式。

## Agent（智能体）状态上报

集成用 `pane.report_agent` 上报 Agent 状态。
...
仅提供 Session 的官方集成用 `pane.report_agent_session` 上报原生 Session 引用。上报状态的集成仍然可以在 `pane.report_agent` 中包含原生 Session 引用。与状态无关的 Session 上报不影响等待、通知或汇总。
...
Herdr 存有原生 Session 引用时，`pane.get`、`pane.list`、`agent.get` 和 `agent.list` 暴露一个只读的 `agent_session` 对象：
...
没有存储原生 Session 引用时，该字段被省略。

当 Herdr 能解析当前控制 Pane PTY 的进程的 cwd 时，`pane.get`、`pane.list`、`agent.get` 和 `agent.list` 也暴露 `foreground_cwd`。已有的 `cwd` 字段仍然是用于标签、follow-cwd 行为和恢复 Session 状态的 Pane/Workspace cwd。
...
元数据上报只影响展示。有效的元数据可以覆盖 Pane 标题、显示的 Agent 名称、紧凑的活动标签和可见的状态标签。`working`、`blocked`、`idle`、等待、通知和汇总仍来自语义状态。原生 Session 恢复来自存储的官方 Session 引用。`agent` 是对权威 Agent 标签的可选守卫；`applies_to_source` 是对活动生命周期权威来源的可选守卫。用 `display_agent` 修改可见名称。`state_labels` 的键必须是 `idle`、`working`、`blocked`、`done` 或 `unknown`。用相同 `source` 搭配 `clear_custom_status: true` 这类清除字段，可以移除某一项展示覆盖。
...
短期元数据用 `ttl_ms`。取值必须在 `1` 到 `86400000` 毫秒之间。想让元数据保留到被替换、清除或 Pane 关闭时，省略 `ttl_ms`。TTL 过期时，Herdr 移除该来源的元数据，并在可见 Pane 展示发生变化时发出展示变化事件。

钩子可能乱序发送更新时，使用 `seq`。对同一 `source`，序号小于等于最后接受序号的上报会被 API 接受，但被 Pane 状态忽略。
...
Workspace 事件订阅包括 `workspace.created`、`workspace.updated`、`workspace.renamed`、`workspace.moved`、`workspace.closed` 和 `workspace.focused`。Workspace 事件描述 Herdr UI/运行时的生命周期。当 Workspace 属于 worktree 组时，`workspace.created` 包含可选的 `workspace.worktree` 来源信息。`workspace.moved` 包含被移动的 `workspace_id`、请求的 `insert_index` 和更新后的有序 `workspaces` 列表。在移除前 Herdr 仍能识别时，`workspace.closed` 包含最终的 `workspace` 快照。

Tab 事件订阅包括 `tab.created`、`tab.closed`、`tab.focused`、`tab.renamed` 和 `tab.moved`。`tab.moved` 包含被移动的 `tab_id`、`workspace_id`、请求的 `insert_index`，以及该 Workspace 更新后的有序 `tabs` 列表。

Pane 事件订阅包括 `pane.created`、`pane.closed`、`pane.focused`、`pane.moved`、`pane.exited`、`pane.agent_detected`、`pane.output_matched`、`pane.agent_status_changed` 和 `pane.scroll_changed`。`pane.scroll_changed` 限定于单个 `pane_id`，每当 Herdr 观察到滚动快照变化时，发出 `pane_id`、`workspace_id` 和当前的 `scroll` 指标。

布局事件订阅包括 `layout.updated`。该事件携带一个 Tab 更新后的 `PaneLayoutSnapshot`。用 `session.snapshot` 引导的客户端应替换具有相同 `workspace_id` 和 `tab_id` 的缓存布局。
...
`detection` 返回 Agent 屏幕检测使用的底部缓冲区快照。
...
`server.agent_manifests` 返回生效的 Agent 检测清单来源和远程更新诊断信息，不重载规则：
...
`agent.explain` 使用服务器生效的清单缓存，在运行中的服务器上评估目标 Pane 的检测快照：
...
响应包含与 `herdr agent explain --json` 打印的相同 explain 对象，包括最终状态、清单来源和版本、匹配的规则、已评估规则的证据、跳过状态原因、idle 回退原因，以及当完整生命周期钩子权威使屏幕规则不再权威时的 `screen_detection_skip_reason`。
...
Herdr 有协议版本号，用于客户端/服务器兼容性判断。协议变更会考虑发布兼容性后再评审。
