...
| Pane | `pane.split`, `pane.list`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_prompt`, `pane.report_recap`, `pane.report_metadata`, `pane.set_header_field`, `pane.clear_header_field`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
...
## Promoted header fields

Use `pane.set_header_field` when a session wants to promote a session-specific fact — a dev container it started, long-task progress, a port, a model name — into its own pane's header as a compact `key value` chip. Chips also surface in the sidebar agent panel, the navigator pane lists, and mobile pane lists, so "which pane has the build at 73%?" is answerable wherever you choose panes.

```json
{
  "id": "req_3",
  "method": "pane.set_header_field",
  "params": {
    "pane_id": "1-1",
    "key": "build",
    "value": "73%",
...
`ttl_secs` is optional: when set, the chip auto-expires (progress that stops updating shouldn't lie). Fields are display-only and ephemeral — they are never persisted into session snapshots, so a restored session starts with a clean header. Caps: 6 fields per pane, key up to 16 characters, value up to 48 characters; over-cap requests fail with an RPC error (`too_many_header_fields`, `invalid_header_field`). Setting an existing key updates it in place.

Remove a chip with `pane.clear_header_field` (idempotent):

```json
{
  "id": "req_4",
  "method": "pane.clear_header_field",
  "params": { "pane_id": "1-1", "key": "build" }
}
```

The `pane_id` resolves like the report methods: a stale env-baked pane id is healed by socket-peer process ancestry, so hooks and wrapper scripts inside the pane can call it without bookkeeping.

## Prompt history scrollback

The pane header keeps a small history of every user prompt the session has reported, plus optional recap entries from a lifecycle hook (e.g. Claude's Stop hook). Collapsed, the header shows the latest user prompt unchanged; expanded (prefix+shift+e or clicking the header), it opens a bounded scrollable panel — history above, latest pinned at the bottom — with the wheel, PageUp/PageDown, Home/End, and Esc all routed to the panel.

`pane.report_prompt` (existing) appends a timestamped prompt entry. `pane.report_recap` appends a recap entry, rendered visually distinct from prompts:
...
```json
{
  ...
  "params": {
    ...
    "source": "herdr:claude",
    "agent": "claude",
    "recap": "fixed the parser bug; tests pass"
  }
}
```

Both feed the same per-pane history ring, capped at roughly 1000 rendered lines (oldest WHOLE entries drop first). The history is ephemeral — never persisted into session snapshots.
