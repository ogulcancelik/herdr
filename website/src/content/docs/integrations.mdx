...
Herdr writes the bundled extension to the agent's extensions directory. Prime Agent (formerly Pi) uses `~/.prime/agent/extensions/` by default, or `~/.pi/agent/extensions/` for older installs:

```text
~/.prime/agent/extensions/herdr-agent-state.ts   # Prime Agent (current)
~/.pi/agent/extensions/herdr-agent-state.ts       # Pi (legacy)
```

If `PRIME_AGENT_CODING_AGENT_DIR` (or the legacy `PI_CODING_AGENT_DIR`) is set, Herdr writes to `$PRIME_AGENT_CODING_AGENT_DIR/extensions/herdr-agent-state.ts` instead. Herdr creates the extensions directory if the agent directory already exists. Uninstall removes only that extension file.

The `pi` and `prime-agent` commands are both detected for agent awareness and integration availability.
