...
`medallion_style` controls the sub-cell raster of the state medallion that leads each server row (the concentric rings showing the worst three agent states on that machine). The default `sextant` uses Unicode Symbols for Legacy Computing for a finer 4x6 grid; set `quadrant` if your terminal font lacks those glyphs.
