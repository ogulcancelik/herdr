...
On macOS this switches to the current ASCII-capable keyboard layout; on Windows it toggles a Korean IME between Hangul and English (other IME languages are detected and left unchanged). Herdr restores the previous input source when returning to terminal input or entering a text field. This setting has no effect on other platforms.
