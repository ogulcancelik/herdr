...
`pane_borders` controls the style of split-pane borders. The default `full` draws a box around every pane and marks the focused pane with an accent-colored border. Set `pane_borders = "minimal"` for a single separator line between adjacent panes, with the inactive panes dimmed (like WezTerm); agent labels are only shown with `full` borders.
