...
`pane.output_matched`, `pane.agent_status_changed`, `pane.scroll_changed`, and
`pane.output_changed`.
...
`pane.output_changed` is scoped to one `pane_id` and emits `pane_id`,
`workspace_id`, and the pane's coalesced output `revision` whenever new
terminal output lands. Events are coalesced, so clients receive the newest
revision rather than one event per write. The same revision is reported by
`pane.read` and pane info responses; `events.wait` accepts `min_revision` to
wait for output strictly newer than a previously read revision.
