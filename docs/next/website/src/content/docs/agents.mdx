...
You can also rename from the sidebar: right-click an agent in the agents panel and choose **Rename** (or **Clear name** to drop a custom label). Because the row already points at one specific agent, the rename applies to exactly that agent without needing a unique name.
