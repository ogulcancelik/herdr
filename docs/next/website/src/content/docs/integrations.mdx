---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, CommandCode, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Cursor Agent CLI, and MastraCode.
---
...
herdr integration uninstall commandcode
...
Install the CommandCode hook:

```bash
herdr integration install commandcode
```

The hook reports CommandCode session identity and working/idle lifecycle hints from `PreToolUse`, `PostToolUse`, and `Stop`. Permission prompts still come from Herdr's screen manifest detection because CommandCode hooks do not emit a separate blocker event while the prompt is waiting.

Herdr uses `~/.commandcode` by default, or `COMMANDCODE_HOME` when set. The CommandCode config directory must already exist. Install writes `hooks/herdr-agent-state.sh` and updates `settings.json` with Herdr hook entries. Uninstall removes the matching hook entries and deletes the hook script.

After CommandCode emits a session-bearing event, Herdr can use the reported session id to resume the pane with `commandcode --resume <id>`.
