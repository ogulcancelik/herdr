---
title: Integrations
description: Install Herdr integrations for Pi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Cursor Agent CLI, MastraCode, and Grok Build.
---
...
Install the Grok Build hook:

```bash
herdr integration install grok
```

The hook reports Grok Build session identity through the same local socket API used by other integrations. Lifecycle state still comes from Herdr's screen manifest detection because Grok Build hooks do not cover every lifecycle transition.

Herdr uses `~/.grok` by default, or `GROK_HOME` when set. The Grok config directory must already exist. Install writes `hooks/herdr-agent-state.sh` and a Herdr-owned `hooks/herdr-agent-state.json` hook config with a `SessionStart` entry; Grok Build loads every `hooks/*.json` file, so no shared settings file is edited. Uninstall deletes both Herdr files and leaves other hook files untouched.

After Grok Build emits a session start event, Herdr can use the reported session id to resume the pane with `grok --resume <id>`.
