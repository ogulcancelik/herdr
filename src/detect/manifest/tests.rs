...
// --- Grok screen rules ---
//
// Evidence: Grok Build 0.2.82 live pane reads (herdr agent read --source
// recent). Snippets below are the bottom lines of the detection buffer.

const GROK_IDLE_TAIL: &str = "\
  ╭──────────────────────────────╮\n\
  │ ❯                            │\n\
...
#[test]
fn grok_spinner_status_line_is_working() {
    let screen = "    ⠧ Waiting on subagent… 2.8s        13s ⇣29.7k [stop]\n";
    let result = explain(Agent::Grok, screen);
    assert_eq!(result.state, AgentState::Working);
    assert_eq!(
        result.matched_rule.as_ref().map(|r| r.id.as_str()),
...
#[test]
fn grok_tool_spinner_status_line_is_working() {
    let screen = "    ⠴ Explore /tmp/grok-detect-lab + 1 more… 5.6s        19s ⇣29.7k [↓][stop]\n";
    let result = explain(Agent::Grok, screen);
    assert_eq!(result.state, AgentState::Working);
    assert_eq!(
        result.matched_rule.as_ref().map(|r| r.id.as_str()),
...
#[test]
fn grok_esc_cancel_hints_are_working() {
    // Redraw gap: footer hints are visible while the spinner line repaints.
    let screen = "  ╰── Claude Fable 5 (xhigh) ────╯\n\
        Enter:send  │  Shift+Tab:mode  │  Esc:cancel  │  Ctrl+Enter:interject  │  Ctrl+.:shortcuts\n";
    let result = explain(Agent::Grok, screen);
    assert_eq!(result.state, AgentState::Working);
    assert_eq!(
        result.matched_rule.as_ref().map(|r| r.id.as_str()),
...
#[test]
fn grok_permission_prompt_is_blocked() {
    let screen = "    ◆ Create empty blocked-test.txt file as re… 7.0s     12s ⇣26.5k [↓][stop]\n\
        \x20 ┃\n\
        \x20 ┃  Create empty blocked-test.txt file as requested.\n\
        \x20 ┃  touch blocked-test.txt\n\
        \x20 ┃\n\
        \x20 ┃  1 (●) Yes, and don't ask again for anything (always-approve mode)\n\
        \x20 ┃  2 (○) Yes, proceed\n\
        \x20 ┃  3 (○) No, reject (type to add feedback)\n\
        \x20 ┃\n\
        \x20 1/3:select  │  Ctrl+o:yolo  │  Ctrl+c:cancel\n";
    let result = explain(Agent::Grok, screen);
    assert_eq!(result.state, AgentState::Blocked);
    assert_eq!(
        result.matched_rule.as_ref().map(|r| r.id.as_str()),
...
#[test]
fn grok_permission_hint_line_alone_is_blocked() {
    // Narrow panes can push the option rows out of the hint region; the
    // footer hint line alone must still classify as blocked.
...
#[test]
fn grok_question_dialog_is_blocked() {
    // ask-user-question dialog; also covers the dialog left up after the
    // turn was cancelled, which still needs a human decision.
    let screen = "    ◆ Waiting on answers for What should I do next?      19s ⇣31.5k [stop]\n\
        \x20 ┃  What should I do next?\n\
        \x20 ┃\n\
        \x20 ┃  1 (○) Make the first commit  Stage notes.md\n\
        \x20 ┃  2 (○) Scaffold a README      Create a README.md\n\
        \x20 ┃  z (○) Type your answer here\n\
        \x20 ┃\n\
        \x20 ┃  ↑/↓ navigate · y copy        Enter:submit\n\
        \x20 ┃\n\
        \x20 Esc:unselect  │  Tab:scrollback  │  Shift+x:dismiss\n";
    let result = explain(Agent::Grok, screen);
    assert_eq!(result.state, AgentState::Blocked);
    assert_eq!(
        result.matched_rule.as_ref().map(|r| r.id.as_str()),
...
#[test]
fn grok_permission_prompt_outranks_parallel_spinner() {
    // Parallel tools can keep a live spinner above a permission prompt; the
    // prompt needs the human, so blocked must win.
    let screen = "    ⠧ Run background task… 8.1s        21s ⇣30.2k [stop]\n\
        \x20 ┃  2 (○) Yes, proceed\n\
        \x20 ┃  3 (○) No, reject (type to add feedback)\n\
        \x20 1/3:select  │  Ctrl+o:yolo  │  Ctrl+c:cancel\n";
    let result = explain(Agent::Grok, screen);
...
#[test]
fn grok_idle_prompt_hints_are_idle() {
    let result = explain(Agent::Grok, GROK_IDLE_TAIL);
    assert_eq!(result.state, AgentState::Idle);
    assert_eq!(
        result.matched_rule.as_ref().map(|r| r.id.as_str()),
...
#[test]
fn grok_idle_transcript_focus_hints_are_idle() {
    let screen = "  ╰── Claude Fable 5 (xhigh) ────╯\n\
        \x20 Ctrl+e:collapse thinking  │  Space:prompt  │  Ctrl+.:shortcuts\n";
    let result = explain(Agent::Grok, screen);
    assert_eq!(result.state, AgentState::Idle);
    assert!(result.visible_idle);
}
...
#[test]
fn grok_splash_braille_logo_is_not_working() {
    // The startup splash draws the logo with braille characters; it must not
    // read as a working spinner.
    let screen = "  ⣼⡟ Grok Build Beta  0.2.82\n\
        \x20 ⣿⡇ v9 is here!\n\
        \x20 Quit                        ctrl+q\n\
        \x20 ╭──────────────╮\n\
        \x20 │ ❯            │\n\
        \x20 ╰──────────────╯\n\
        \x20 xAI Internal - Grok Business  │  [stable]\n";
    let result = explain(Agent::Grok, screen);
    assert_eq!(result.state, AgentState::Idle);
    assert_eq!(
        result.fallback_reason.as_deref(),
...
#[test]
fn grok_legacy_scope_selector_is_blocked() {
    let screen = "Yes, proceed\nNo, reject\nuse ← → to choose permission whitelist scope\n";
    let result = explain(Agent::Grok, screen);
    assert_eq!(result.state, AgentState::Blocked);
    assert_eq!(
        result.matched_rule.as_ref().map(|r| r.id.as_str()),
        Some("permission_scope_selector")
    );
}
