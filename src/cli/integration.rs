...
fn parse_integration_target(
    ...
    let Some(target) = args.first().map(|arg| arg.as_str()) else {
        eprintln!(
            "usage: herdr integration {action} <pi|omp|claude|codex|copilot|devin|opencode|hermes|qodercli>"
        );
        return Ok(None);
    };
    if args.len() != 1 {
    ...
    let parsed = match target {
        ...
        _ => {
            ...
            eprintln!(
                "currently supported: pi, omp, claude, codex, copilot, devin, opencode, hermes, qodercli"
            );
            return Ok(None);
        }
    };
