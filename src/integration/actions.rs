...
use super::targets::{
    install_claude, install_codex, install_commandcode, install_copilot, install_cursor,
    install_devin, install_droid, install_hermes, install_kilo, install_kimi, install_mastracode,
    install_omp, install_opencode, install_pi, install_qodercli, uninstall_claude, uninstall_codex,
    ...
    let mut messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Commandcode => {
            ...
            vec![
                ...
                format!(
                    "ensured commandcode settings at {}",
                    installed.settings_path.display()
                ),
            ]
        }
...
pub(crate) fn uninstall_target(
    ...
    let messages = match target {
        ...
        crate::api::schema::IntegrationTarget::Commandcode => {
            ...
            if result.updated_settings {
                messages.push(format!(
                    "removed herdr commandcode hook entries from {}",
                    result.settings_path.display()
                ));
            } else {
                messages.push(format!(
                    "no herdr commandcode hook entries found in {}",
                    result.settings_path.display()
                ));
            }
            messages
