...
### Added
- Added optional `keys.apply_project_layout` for spawning a per-project layout script (default filename `.herdr-project`) located in the focused pane's working directory. The script receives the same env vars as `[[keys.command]]` plus `HERDR_PROJECT_LAYOUT_FILE` and `HERDR_PROJECT_ROOT`. Filename configurable via `keys.project_layout_filename`.
